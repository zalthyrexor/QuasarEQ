...
namespace zlth::dsp{
  ...
  private:
    ...
    FORCEINLINE static void calculate_coefficients(config::FilterType f_, float g_, float k_, float a1_, Setter&& set) noexcept {
      switch (f_) {
        ...
        case config::FilterType::Bell:
        {
          ...
          set(g_, k_ / a2_, 1.0f, k_ * (a4_ - 1.0f) / a2_, 0.0f);
          break;
        }
        case config::FilterType::LowShelf:
        {
          const float a2_ {a1_ * a1_};
          const float a4_ {a2_ * a2_};
          set(g_ / a1_, k_, 1.0f, k_ * (a2_ - 1.0f), a4_ - 1.0f);
          break;
        }
        case config::FilterType::HighShelf:
        {
          const float a2_ {a1_ * a1_};
          const float a4_ {a2_ * a2_};
          set(g_ * a1_, k_, a4_, k_ * (a2_ - a4_), 1.0f - a4_);
          break;
        }
        case config::FilterType::Tilt:
        {
          const float a2_ {a1_ * a1_};
          const float a4_ {a2_ * a2_};
          set(g_ * a1_, k_, a2_, k_ * (1.0f - a2_), (1.0f - a4_) / a2_);
          break;
        }
    ...
    FORCEINLINE void process_single(float& v0) {
      const float v1 {a1 * (ic1eq + g * (v0 - ic2eq))};
