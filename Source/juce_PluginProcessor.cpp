...
QuasarEQAudioProcessor::QuasarEQAudioProcessor():AudioProcessor(BusesProperties().
  ...
  for (int i = 0; i < config::BIQUAD_COUNT; ++i) {
    auto load = [&](int index) {
      return apvts.getRawParameterValue(config::toBiquadID(config::biquadPrefixes[(int)index], i));
    };
    auto* l0_ = load(config::PIdx::Freq);
    auto* l1_ = load(config::PIdx::Q);
    auto* l2_ = load(config::PIdx::Gain);
    auto* l3_ = load(config::PIdx::Bypass);
    auto* l4_ = load(config::PIdx::Type);
    auto* l5_ = load(config::PIdx::Chan);
    processors.push_back({
      zlth::dsp::Filter(l0_, l1_, l2_, l3_, l4_, l5_, sr_, 0),
      zlth::dsp::Filter(l0_, l1_, l2_, l3_, l4_, l5_, sr_, 1)
      }
    );
  }
  ...
  for (int i = 0; i < config::BUTTER_COUNT; ++i) {
    auto load = [&](int index) {
      return apvts.getRawParameterValue(config::toButterID(config::butterPrefixes[(int)index], i));
    };
    auto* l0_ = load(0);
    auto* l1_ = load(1);
    auto* l2_ = load(2);
    ...
    auto* l4_ = load(4);
    processors.push_back({
      zlth::dsp::Butterworth(l0_, l1_, l2_, l3_, l4_, sr_, 0),
      zlth::dsp::Butterworth(l0_, l1_, l2_, l3_, l4_, sr_, 1)
      }
    );
  }
}
...
void QuasarEQAudioProcessor::initializeAllParameters() const {
  ...
  for (int i = 0; i < config::BIQUAD_COUNT; ++i) {
    for (int j = 0; j < config::biquadPrefixes.size(); ++j) {
      resetParam(config::toBiquadID(config::biquadPrefixes[j], i));
    }
  }
  for (int i = 0; i < config::BUTTER_COUNT; ++i) {
    for (int j = 0; j < config::butterPrefixes.size(); ++j) {
